...
const char * const BITCOIN_CONF_FILENAME = "benchcoin.conf";
...
#else
    const char* pszModule = "benchcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Benchcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Benchcoin";
#else
    // Unix
    return pathRet / ".benchcoin";
#endif
#endif
}
